...
1. The API is too busy to answer
    * Since there is no load balancing etc. at this point ([read](https://confluence.broadinstitute.org/display/GDAC/mail/66356369)), there might be some problems with big queries, in special when querying for large data (e.g. expression), for hundreds of patients. Here you might consider further chunking and not only use the page/page_size feature.
